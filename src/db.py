...
@contextmanager
def db_execute(write: bool = False) -> Iterator[sqlite3.Connection]:
    """
    统一连接生命周期：只读不发起 commit；写入在成功退出时 commit，异常时 rollback。
    """
    conn = get_conn()
    try:
        yield conn
...
def init_db():
    ...
    with db_execute(write=True) as conn:
        c = conn.cursor()
        # 鸭鸭档案表
...
def get_profile(user_id: str) -> Optional[Dict[str, Any]]:
    with db_execute(write=False) as conn:
        c = conn.cursor()
